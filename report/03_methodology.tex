...
We use the HotpotQA dataset \citep{yang2018hotpotqa} in its distractor setting as the base benchmark because it satisfies three properties relevant to this study. First, it requires multi-hop reasoning over multiple context passages, which reflects the retrieval scenarios in which enterprise RAG systems often operate and are most vulnerable. Second, it provides ground-truth supporting passage indices and distractor passages per question, which lets us label poisoned passage indices exactly for supervised pre-filter training and for the True/Survived decomposition introduced in Section~\ref{sec:true-survived}. Third, it is the base benchmark used in the original PoisonedRAG evaluation \citep{zou2025poisonedrag}, which permits direct comparison with prior work on RAG security. We wanted a benchmark that satisfied all the requirements and was already recognised in prior work.

We sample a set of 100 HotpotQA hard questions whose answers require two supporting passages. These 100 base questions form the foundation of all 2{,}400 evaluation triplets in the final dataset. We choose 100 base questions to keep the full three-judge, multi-condition evaluation feasible while preserving balanced cells across attack type and noise level.
...
For each poisoned triplet, the indices of replaced passages are recorded in a \texttt{poisoned\_\allowbreak passage\_\allowbreak indices} field, denoted $\mathcal{P} \subseteq \{1, \ldots, n\}$. This explicit per-passage labelling enables supervised pre-filter training and residual-aware analysis. The dataset fields are summarised in \hyperref[app:schema]{Appendix D}.

We keep paired clean controls so each poisoned triplet can be compared against the same question without poisoning.
...
\caption{PoisonedRAG-style poisoned-context construction process, showing target wrong-answer generation, malicious passage synthesis, and additive insertion into the fixed HotpotQA context. The construction preserves the original supporting passages and does not evaluate retrieval reranking or downstream answer generation.}
...
The pre-filter classifier is trained on the train split, hyperparameters are selected on the validation split, and all classifier-level performance is reported on the held-out test split.
...
For PoisonedRAG-style injections, quality assurance focuses on construction validity rather than a full retrieval-generation evaluation. We verify that each malicious passage is topically aligned with the question, supports the generated wrong target answer $a^{\dagger}$, and is inserted additively so that original supporting passages are preserved. We do not evaluate retrieval reranking or downstream answer generation in this thesis; the resulting poisoned contexts are used directly for judge-robustness evaluation.
...
We evaluate three current LLM judges spanning different provider and deployment profiles. We made the decision to diversify into different segments of current state-of-the-art models while keeping the experiment computationally tractable. GPT-5.4-nano represents the proprietary OpenAI judge in the panel. Hence, we used Gemma as a recent open-weight model from Google, and DeepSeek as a lower-cost model family with a different provider profile from OpenAI.
...
The models are chosen to cover different provider types rather than to exhaust the model space. Detailed configuration values are recorded in \hyperref[app:config]{Appendix F}.
...
We adhere to the RAGAS conventions for the three metrics. \textbf{Faithfulness} measures whether the generated answer is supported by the provided context. \textbf{Answer relevance} measures whether the generated answer is relevant to the question. \textbf{Context relevance} measures whether the retrieved context is relevant to the question. Faithfulness is the primary metric of interest because it is the dimension that corpus poisoning compromises most directly.
...
The threshold corresponds to the natural midpoint of the scoring interval and is the conventional operating point for binarising RAGAS scores.
...
Both pre-filter modules are designed against four specification points: (i) integration as a drop-in pipeline component without modifying the judge prompt, (ii) computational tractability, (iii) multi-signal redundancy for the statistical filter, and (iv) controllable sensitivity.

\subsection{Statistical pre-filter: five signals}

The statistical pre-filter is a passage-level binary classifier. For each retrieved passage $p_{i} \in C$, the filter computes five signals, aggregated into a per-passage poison probability $\hat{y}_{i} \in [0, 1]$. Full configuration values are listed in \hyperref[app:config]{Appendix F}.

\textbf{Signal 1, embedding cosine anomaly.} This signal compares the passage embedding with the question embedding and flags unusually distant or unusually high-similarity passages. It was the first signal we thought of, a classical NLP-style signal which may be efficient for noisy or abnormal passage detection.

\textbf{Signal 2, token-level entropy.} This signal uses language-model uncertainty to identify passages with unusual token-level probability patterns. We thought this might help because poisoned or generated passages may have different uncertainty patterns compared to normal retrieved text.

\textbf{Signal 3, fine-tuned classifier.} A lightweight transformer model is fine-tuned as a binary classifier on the training split to output a direct poison probability. We included this signal to see whether a small supervised model could directly learn the poisoning patterns from labelled examples as we had access to labelled data.

\textbf{Signal 4, answer span recall.} This signal measures whether a passage preserves tokens associated with the ground-truth answer. We believed that this signal would be informative, as an authentic supporting passage should maintain at least some link to the ground-truth answer, unlike a poisoned passage that might be intended to destroy or weaken it.

\textbf{Signal 5, cross-encoder relevance.} This signal scores query-passage relevance using an interaction-based cross-encoder. We included this signal because poisoned passages can still look relevant to the question, so relevance alone may not be enough, but it could still add useful information when combined with the other signals.
...
The best-performing aggregator on the validation set is used as the primary aggregator for end-to-end experiments, with Weighted Vote retained as an unbiased baseline for ablation analysis.
...
For Signal~3, we compare two encoder backbones: DeBERTa-v3-small and RoBERTa-base. Both are fine-tuned on the same train split with matched hyperparameters, listed in \hyperref[app:config]{Appendix F}.

\subsection{LLM pre-filter: triplet-level classification}

The second pre-filter strategy employs Mistral-small-latest as the LLM classifier. This model is not part of either the poison-generation model family or the judge model families, reducing in-family recognition confounds. We used a different model family here to avoid the judge and filter coming from the same family.
...
The output is a set of flagged indices $\hat{\mathcal{P}} \subseteq \{1, \ldots, n\}$. Flagged passages are removed from the context before the judge scores the triplet, producing a filtered context $C^{\prime} = C \setminus \{p_{i} : i \in \hat{\mathcal{P}}\}$. All triplets are scored regardless of whether any passages were flagged; the key distinction from the statistical filter is the signal source (holistic triplet-level reasoning by an LLM rather than per-passage signal aggregation) and the resulting removal rate (on average 1.2 passages removed per triplet, compared to 4.5 for the statistical filter). The two strategies use different detection signals and produce different residuals, which is why we evaluate both.
...
For the LLM filter, the output is a set of flagged passage indices. Flagged passages are removed and the judge scores the resulting filtered context, identically to the statistical filter pipeline.
...
The evaluation pipeline is implemented in Python using version-controlled configuration files. The implementation records raw judge outputs, pre-filter outputs, and analysis exports in structured files so that tables and figures can be regenerated from saved artefacts. The final configuration is summarised in \hyperref[app:config]{Appendix F}, and the reproducibility workflow is summarised in \hyperref[app:reproducibility]{Appendix G}.
...
To account for inter-triplet dependencies stemming from the shared base questions, question-clustered bootstrap resampling is used for paired significance checks where base-question dependence matters (1{,}000 iterations). ROC curves are computed using scikit-learn's \texttt{roc\_curve} function, with the optimal operating point selected using the Youden index ($\mathrm{sensitivity} + \mathrm{specificity} - 1$).
