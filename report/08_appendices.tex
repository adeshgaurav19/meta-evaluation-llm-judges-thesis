...
\chapter{Evaluation Prompt Templates}
...
\chapter{Attack Construction Prompts}
...
\begin{description}[leftmargin=3.4cm, style=nextline]
    ...
    \item[LLM Filter]
    The triplet-level Mistral filter that inspects the full question-context-answer
    triplet, flags suspected adversarial passage indices, and removes those
    passages before judge scoring.
...
\chapter{Poisoned Dataset Schema and Examples}
\label{app:schema}
...
Representative examples for the three injection types are stored in the fixed
PoisonedRAG data directory. The final analysis tables derived from these
triplets are exported in \texttt{exports/}, which is the canonical result
directory used for the submitted analysis.

\chapter{Additional Experimental Results and Ablation Tables}
...
inter-judge-variance tables, length correlations, collateral-damage audits,
per-signal ablations, per-injection signal ablations, classifier-backbone
comparisons, and the distractor-stratified paradox analysis. Thesis figures are
stored under \texttt{exports/figures/}.

Table~\ref{tab:appendix-signal-ablation} reports validation behaviour under a
reduced ablation protocol and is used only to compare the relative contribution
of signals within that protocol. It is not directly comparable to the held-out
test-set classifier performance reported in Table~\ref{tab:stat-filter-performance}.

\begin{table}[H]
\centering
\small
\caption{Minimal per-signal ablation report under the reduced validation ablation protocol. Each row removes one signal from the statistical pre-filter and recomputes validation performance.}
\label{tab:appendix-signal-ablation}
\begin{tabular}{lrrrr}
\toprule
Removed signal & Precision & Recall & F1 & $\Delta$F1 \\
\midrule
Embedding cosine & 0.354 & 0.483 & 0.408 & +0.000 \\
Entropy & 0.315 & 0.577 & 0.407 & -0.001 \\
Classifier & 0.289 & 0.548 & 0.378 & -0.030 \\
Cross-encoder & 0.299 & 0.518 & 0.379 & -0.029 \\
Answer span recall & 0.326 & 0.486 & 0.390 & -0.018 \\
None (all signals) & 0.315 & 0.577 & 0.408 & 0.000 \\
\bottomrule
\end{tabular}
\end{table}
...
\centering
\small
\caption{Classifier backbone ablation. Both models are trained on the same split with matched hyperparameters.}
\label{tab:appendix-classifier-ablation}
\begin{tabular}{lr}
\toprule
Classifier backbone & Validation F1 \\
\midrule
DeBERTa-v3-small & 0.7215 \\
RoBERTa-base & 0.7202 \\
\bottomrule
\end{tabular}
\end{table}
...
to \texttt{exports/}. The code repository is available online as the
\href{https://github.com/adeshgaurav19/meta-evaluation-llm-judges-thesis}{GitHub repository}.
API keys are loaded from environment variables and are not included in the
repository.
